...
describe("users.onboardUser", () => {
  test("rejects unauthenticated users", async () => {
    ...
    await expect(t.mutation(api.users.onboardUser, onboardingArgs)).rejects.toThrow(
      "Not signed in",
    );
  });
  ...
  test("rejects authenticated users without a user record", async () => {
    ...
    await expect(asUser.mutation(api.users.onboardUser, onboardingArgs)).rejects.toThrow(
      "User not found",
    );
  });
