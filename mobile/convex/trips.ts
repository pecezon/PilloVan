...
import { getCurrentCompanyUserOrThrow, getCurrentUserOrThrow, userByEmail } from "./lib/users";
...
export const createTrip = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.partySize > tour.occupancy) {
      throw new ConvexError(`Party size exceeds the tour occupancy of ${tour.occupancy}`);
    }
    ...
    if (missingEmails.length > 0) {
      throw new ConvexError(`No account found for: ${missingEmails.join(", ")}`);
    }
