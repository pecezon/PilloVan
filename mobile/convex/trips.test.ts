...
describe("trips.createTrip", () => {
  test("rejects unauthenticated callers", async () => {
    ...
    await expect(
      t.mutation(api.trips.createTrip, tripArgs(tourId))
  ...
  test("rejects a tour owned by another company", async () => {
    ...
    await expect(
      t.withIdentity(otherCompany).mutation(
        api.trips.createTrip,
        tripArgs(tourId, { participantEmails: [] })
      )
  ...
  test("rejects unknown participant emails", async () => {
    ...
    await expect(
      t.withIdentity(companyIdentity).mutation(
        api.trips.createTrip,
  ...
  test("rejects party size larger than tour occupancy", async () => {
    ...
    await expect(
      t.withIdentity(companyIdentity).mutation(
        api.trips.createTrip,
        tripArgs(tourId, { partySize: 99, participantEmails: [] })
      )
