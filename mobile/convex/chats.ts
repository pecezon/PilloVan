  ...
  handler: async (ctx, args) => {
    ...
    for (const kind of CHAT_KINDS) {
      const existing = await ctx.db
        .query("chats")
        .withIndex("by_tripId_and_kind", (q) => q.eq("tripId", args.tripId).eq("kind", kind))
        .unique();
      if (!existing) {
...
export const listMessages = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (const message of result.page) {
      ...
      page.push({
        ...message,
        sender: sender ? { firstName: sender.firstName, lastName: sender.lastName, role } : null,
      });
    }
...
export const sendMessage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (body.length > MAX_MESSAGE_LENGTH) {
      throw new ConvexError(`Messages must be ${MAX_MESSAGE_LENGTH} characters or fewer`);
    }
