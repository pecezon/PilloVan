...
function ChatArea({ tripId, myId }: { tripId: Id<"trips">; myId: Id<"users"> | undefined }) {
  ...
  const activeChat = chats.find((chat) => chat.kind === selectedKind) ?? chats[0];
    ...
    <VStack className="flex-1">
      ...
      {activeChat ? (
        ...
        <Box className="flex-1 items-center justify-center px-6">
          <Text className="text-center text-typography-500">No chat available for this trip.</Text>
        </Box>
      )}
    </VStack>
  );
...
function MessageList({ chat, myId }: { chat: ChatDoc; myId: Id<"users"> | undefined }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const message = error instanceof ConvexError ? String(error.data) : "Could not send message.";
      toast.show({
        placement: "top",
        render: ({ id }) => (
  ...
  return (
    ...
    >
      {results.length === 0 && status !== "LoadingFirstPage" ? (
        <Box className="flex-1 items-center justify-center px-6">
          <Text className="text-center text-typography-500">No messages yet. Say hello!</Text>
        </Box>
      ) : (
        <FlatList
          data={results}
...
function MessageBubble({ message, isMine }: { message: MessageItem; isMine: boolean }) {
  ...
  return (
    <Box className={`mb-2 max-w-[80%] ${isMine ? "self-end" : "self-start"}`}>
      ...
      <Box className={`rounded-2xl px-4 py-2 ${isMine ? "bg-primary-500" : "bg-background-100"}`}>
        <Text className={isMine ? "text-typography-0" : "text-typography-900"}>{message.body}</Text>
      </Box>
      <Text
        className={`mt-1 text-[10px] text-typography-400 ${isMine ? "text-right" : "text-left"}`}
      >
